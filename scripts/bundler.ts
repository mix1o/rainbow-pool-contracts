...
const config = {
  paths: {
    outDir: join(ROOT, "package"), // Path to root folder
    deployments: join(ROOT, "deployments"), // Path to folder with deployments (compatible with hardhat-deploy only)
    artifacts: join(ROOT, "artifacts"), // Path to folder with artifacts to be copied
  },
  addresses: {
  ...
  for (const deployment of deployments) {
    ...
    if (!config.addresses.ignoredDeployments.includes(deployment)) {
      ...
      if (deploymentContent.includes(".chainId")) {
        chainId = (
          await readFile(join(config.paths.deployments, deployment, ".chainId"))
        ).toString();
      } else {
        throw new Error(
          `Missing ${join(config.paths.deployments, deployment, ".chainId")}`
        );
      }
      ...
      for (const fileName of deploymentContent) {
        ...
        if (
          ...
          const fileContent = (
            await readFile(join(config.paths.deployments, deployment, fileName))
          ).toString();
...
const createIndexFile = async () => {
  // Write imports
  const writeImports = [
    `import addresses from './${config.addresses.fileName}.json';`,
  ].join("\n\t");

  // Write reexports
  const writeReexports = [`export * from './typechain/index';`].join("\n\t");
...
`;
  ...
  await writeFile(join(config.paths.outDir, "src", "index.ts"), writeIndexFile);
};
...
const run = async () => {
  ...
  log("Running tsc...");
  await execAsync(
    `tsc --declaration true --target es2017 --moduleResolution node --resolveJsonModule true --outDir ${join(
      config.paths.outDir,
      "dist"
